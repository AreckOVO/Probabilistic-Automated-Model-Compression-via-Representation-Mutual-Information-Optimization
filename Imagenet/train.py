...
parser.add_argument('--dataset', type=str, default='imagenet',
                    choices=['imagenet', 'imagenet10', 'imagenet100'])
parser.add_argument('--save_path', type=str, default='/home/data/imagenet') #放数据集的地方
parser.add_argument('--train_batch_size', type=int, default=256)
parser.add_argument('--test_batch_size', type=int, default=250)

""" model config """
parser.add_argument('--model', type=str, default="resnet18",
                    choices=['resnet18', 'resnet34', 'resnet50_TAS', 'mobilenetv2_autoslim', 'resnet50', 'mobilenetv2', 'mobilenet'])
    ...
    if args.resume: #True or False，加载resume模型，包括权重、epoch、best_acc和optimizer
        ...
        if args.model == 'resnet18':
            run_manager.reset_model(ResNet_ImageNet(
                num_classes=run_config.data_provider.n_classes, cfg=eval(args.cfg), depth=18), model_origin = net_origin.cpu(), quant_cfg=eval(args.quant_cfg), args=args)
        elif args.model == 'resnet34':
            run_manager.reset_model(ResNet_ImageNet(
                num_classes=run_config.data_provider.n_classes, cfg=eval(args.cfg), depth=34), model_origin = net_origin.cpu(), quant_cfg=eval(args.quant_cfg), args=args)
        elif args.model == 'resnet50': 
            run_manager.reset_model(ResNet_ImageNet( #模型初始化、剪枝、定义criterion和optimizer
                num_classes=run_config.data_provider.n_classes, cfg=eval(args.cfg), depth=50), model_origin = net_origin.cpu(), quant_cfg=eval(args.quant_cfg), args=args) #eval函数可以把str转化为list、dict、tuple
        elif args.model == 'mobilenet':
            run_manager.reset_model(MobileNet(
                num_classes=run_config.data_provider.n_classes, cfg=eval(args.cfg)), model_origin = net_origin.cpu(), quant_cfg=eval(args.quant_cfg), args=args)
        elif args.model == 'mobilenetv2':
