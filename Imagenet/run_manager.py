...
class RunConfig:
    ...
    def build_optimizer(self, net_params): #net_params是个list，list中第一个元素是使用weight decay的parameter，第二个元素是不使用weight decay的parameter
        if self.opt_type == 'sgd':
            opt_param = {} if self.opt_param is None else self.opt_param
            momentum, nesterov = opt_param.get( #从opt_param中获得优化器参数momentum和nesterov
                'momentum', 0.9), opt_param.get('nesterov', True)
            if self.no_decay_keys:
                optimizer = torch.optim.SGD([
                    {'params': net_params[0], #这部分使用weight decay
                        'weight_decay': self.weight_decay},
                    {'params': net_params[1], 'weight_decay': 0}, #这部分不使用weight decay
...
class RunManager:
    def __init__(self, path, net, run_config: RunConfig, out_log=True):
        ...
        self.net = DDP(self.net, delay_allreduce=True) #apex的DDP
    ...
    def reset_model(self, model, model_origin=None, quant_cfg=None, args=None): #模型初始化、剪枝、定义criterion和optimizer
        self.net = model
        self.net.init_model(self.run_config.model_init, #对net进行初始化，model_init是初始化方法，包括"he_fin"、"xavier_normal"、"xavier_uniform"
                            self.run_config.init_div_groups)
        #剪枝
        if model_origin is not None:
            if self.run_config.local_rank == 0:
                print('-'*30+' start pruning '+'-'*30)
            get_unpruned_weights(self.net, model_origin) #根据model_origin中的预训练权重，对net进行剪枝
            if self.run_config.local_rank == 0:
                print('-'*30+' end pruning '+'-'*30)
        #量化
        if quant_cfg != None:
            if self.run_config.local_rank == 0:
                print('-'*30+' start quantizing '+'-'*30)
            self.net.quantize_net(quant_cfg, args) 
            if self.run_config.local_rank == 0:
                print('-'*30+' end quantizing '+'-'*30)
        self.net = self.net.cuda()
        # net info
        if self.run_config.local_rank == 0:
            self.print_net_info() #打印网络信息：参数量、flops等

        #Apex实现了同步BN，用于解决单GPU的minibatch太小导致BN在训练时不收敛的问题
        if self.run_config.sync_bn:
            self.net = apex.parallel.convert_syncbn_model(self.net)
        print('local_rank: %d' % self.run_config.local_rank)
        ...
        self.criterion = nn.CrossEntropyLoss()
        if self.run_config.no_decay_keys:
            keys = self.run_config.no_decay_keys.split('#')  # bn#bias，哪些模块不适用weight decay
            self.optimizer = self.run_config.build_optimizer([
                # parameters with weight decay
                self.net.get_parameters(keys, mode='exclude'), #这部分使用weight decay
                # parameters without weight decay
                self.net.get_parameters(keys, mode='include'), #这部分不使用weight decay
            ]) 
        else:
            self.optimizer = self.run_config.build_optimizer( #所有模块都使用weight decay
                self.net.weight_parameters())
        ...
        #                                        loss_scale=1.0
        #                                        )
        self.net = DDP(self.net, delay_allreduce=True) #把网络包装成DDP对象
        cudnn.benchmark = True
    ...
    def print_net_info(self): #输出网络信息，包括参数量、flops和config
        # parameters
    ...
    def load_model(self, model_fname=None):
        ...
        try:
            ...
            if torch.cuda.is_available(): #加载checkpoint
                checkpoint = torch.load(model_fname)
            else:
                checkpoint = torch.load(model_fname, map_location='cpu')

            self.net.module.load_state_dict(checkpoint['state_dict']) #把checkpoint中的权重读取到net
            # set new manual seed
            new_manual_seed = int(time.time())#设置随机种子
            torch.manual_seed(new_manual_seed)
            torch.cuda.manual_seed_all(new_manual_seed)
            np.random.seed(new_manual_seed)

            if 'epoch' in checkpoint: #读取epoch
                self.start_epoch = checkpoint['epoch'] + 1
            if 'best_acc' in checkpoint: #读取best_acc
                self.best_acc = checkpoint['best_acc']
            if 'optimizer' in checkpoint: #读取optimizer
