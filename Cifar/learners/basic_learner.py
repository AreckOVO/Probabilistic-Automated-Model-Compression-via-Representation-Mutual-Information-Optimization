...
class BasicLearner(object):
	...
	def train(self, train_sampler=None, save_whole_model=False):
		self.warm_up_lr()
		for epoch in range(self.args.epochs):
			...
			if (epoch+1) % self.args.eval_epoch == 0:
				...
				if self.check_is_primary():
					self.save_model(save_whole_model=save_whole_model)
	...
	def finetune(self, train_sampler, save_whole_model=False):
		self.load_model()
		self.evaluate()
	...
	def save_model(self, save_whole_model = False):
		if save_whole_model:
