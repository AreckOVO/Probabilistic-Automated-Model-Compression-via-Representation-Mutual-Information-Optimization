...
class ResNetDecode(nn.Module):
    ...
    def quantize_net(self, quant_cfg, args):
        # self.conv1 = quantize_conv(self.conv1, 8, args)#第一层量化为8bit
        # self.conv1.weight_quantizer.bitwidth_refactor(refactored_bit = 8)
        # self.conv1.activation_quantizer.bitwidth_refactor(refactored_bit = 8)
        # pdb.set_trace()
    ...
    def quantize_layer(self, w_bits, idx, args):
        ...
        if idx == 0:
            self.conv1 = quantize_conv(self.conv1, w_bits, args)#第一层量化为8bit
            self.conv1.weight_quantizer.bitwidth_refactor(refactored_bit = 32)
            self.conv1.activation_quantizer.bitwidth_refactor(refactored_bit = 32)
            return
        elif idx in range(1, 1+2*num_blocks[0]):
            setattr(self.layer1[(idx - 1) // 2],'conv%d'%((idx - 1) % 2 + 1),quantize_conv(getattr(self.layer1[(idx - 1) // 2], 'conv%d'%((idx - 1) % 2 + 1)), w_bits, args))
            if (idx - 1) // 2 == 0:
              self.layer1[0].shortcut.conv = quantize_conv(self.layer1[0].shortcut.conv, w_bits, args)
        elif idx in range(1+2*num_blocks[0], 1+2*num_blocks[0]+2*num_blocks[1]):
            setattr(self.layer2[(idx - (1+2*num_blocks[0])) // 2], 'conv%d'%((idx - (1+2*num_blocks[0])) % 2 + 1), quantize_conv(getattr(self.layer2[(idx - (1+2*num_blocks[0])) // 2], 'conv%d'%((idx - (1+2*num_blocks[0])) % 2 + 1)), w_bits, args))      
            if (idx - (1+2*num_blocks[0])) // 2 == 0:
              self.layer2[0].shortcut.conv = quantize_conv(self.layer2[0].shortcut.conv, w_bits, args)
        elif idx in range(1+2*num_blocks[0]+2*num_blocks[1], 1+2*num_blocks[0]+2*num_blocks[1]+2*num_blocks[2]):
            setattr(self.layer3[(idx - (1+2*num_blocks[0]+2*num_blocks[1])) // 2], 'conv%d'%((idx - (1+2*num_blocks[0]+2*num_blocks[1])) % 2 + 1), quantize_conv(getattr(self.layer3[(idx - (1+2*num_blocks[0]+2*num_blocks[1])) // 2], 'conv%d'%((idx - (1+2*num_blocks[0]+2*num_blocks[1])) % 2 + 1)), w_bits, args))
            if (idx - (1+2*num_blocks[0]+2*num_blocks[1])) // 2 == 0:
              self.layer3[0].shortcut.conv = quantize_conv(self.layer3[0].shortcut.conv, w_bits, args)
