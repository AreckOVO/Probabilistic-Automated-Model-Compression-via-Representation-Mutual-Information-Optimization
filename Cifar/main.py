...
parser_learner.add_argument('--save_whole_model', default=False, help='whether to store whole model')
...
parser_arc.add_argument('--quant_cfg', default='', help='quantization_cfg')
...
def main():
	...
	try:
		...
		if args.rank == 0:
			...
			# setup logging
			if args.exec_mode in ['train', 'misc']: #训练时走这条
				args.job_id = generate_job_id() #生成一串随机的数字
				init_logging(os.path.join(args.save_path, '_'.join([args.model_type, args.learner]), args.job_id, 'record.log'))#./models/resnet_decode_vanilla/Z5hMBJaH/record.log
		...
		if args.quant_cfg != '':
			args.quant_cfg = list(eval(args.quant_cfg))
			model.quantize_net(args.quant_cfg, args)
		model.cuda()
		...
		loaders, sampler = set_loader(args)#获取dataloader
		learner_fn = set_learner(args)
		learner = learner_fn(model, loaders, args, device)

		if args.exec_mode == 'train':
			learner.train(sampler, args.save_whole_model)
		elif args.exec_mode == 'eval':
			learner.evaluate()
		elif args.exec_mode == 'finetune':
			learner.finetune(sampler, args.save_whole_model)
